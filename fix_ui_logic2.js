...
const targetStr = `                  <div style={{ marginBottom: '10px' }}>
                    <label style={{ fontSize: '11px', fontWeight: 'bold', display: 'block', marginBottom: '4px' }}>إجمالي الفاتورة:</label>
                    <div style={{ fontSize: '14px', fontWeight: 'bold', color: theme.textDark }}>
                      {(invoices.find(i => i.id === payTargetInvoiceId)?.totalAmount || 0)} {t.currency}
                    </div>
                  </div>
                  ...
                  <div>
                    <label style={{ fontSize: '11px', fontWeight: 'bold', display: 'block', marginBottom: '4px' }}>المبلغ المتبقي:</label>
                    <div style={{ fontSize: '14px', fontWeight: 'bold', color: '#ef4444' }}>
                      {Math.max(0, (invoices.find(i => i.id === payTargetInvoiceId)?.totalAmount || 0) - (Number(partialPayAmount) || 0)).toFixed(2)} {t.currency}
                    </div>
                  </div>`;
...
if (content.includes(targetStr)) {
  ...
  console.log('Successfully updated the exact string.');
} else {
