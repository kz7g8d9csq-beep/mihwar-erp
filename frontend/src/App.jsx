...
function App() {
  ...
  // الموارد البشرية: دالة إعفاء وإلغاء الخصم المحددة المطلوبة
  const handleRemoveDeduction = (deductId, empName, amount) => {
    ...
    // استرجاع وتحديث رصيد الموظف (طرح مبلغ الخصم المعفى من إجمالي خصوماته)
    const updatedEmps = employees.map(emp => {
      if (emp.name === empName) {
  ...
  const handleAddCustomer = async (e) => {
    ...
    try {
      ...
      setCustName(''); setCustNationalId(''); setCustPhone(''); fetchCustomers();
    } catch (e) { alert('Error'); }
  };
  ...
  const handleAddSupplier = async (e) => {
    ...
    try {
      ...
      setSuppName(''); setSuppTaxNumber(''); setSuppPhone(''); fetchSuppliers();
    } catch (e) { alert('Error'); }
  };
        ...
        <main style={{ padding: '30px', flex: 1, boxSizing: 'border-box' }}>
          ...
          {activeTab === 'hr' && user.role !== 'cashier' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '25px' }}>
              ...
              {/* إضافة زر إعفاء الخصم في سجل الخصومات التفصيلي */}
              <div style={{ background: theme.cardBg, borderRadius: '16px', border: `1px solid ${theme.border}`, padding: '25px', overflowX: 'auto' }}>
